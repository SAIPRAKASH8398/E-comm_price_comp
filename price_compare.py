...
class price_compare:
        ...
        def find(self):
            ...
            try:
                ...
                self.variable_amzn.set("product not avaliable")

            try:
                self.variable_flip.set(self.matches_flip[0])
            except:
                self.variable_flip.set("product not avaliable")

            option_amazon = OptionMenu(self.window, self.variable_amzn, *self.matches_amzn)
            option_amazon.grid(row=3, column=1, sticky=W)

            lab_amzn = Label(self.window, text="not this ? try out suggestions by clicking on title")
            lab_amzn.grid(row=3, column=2, padx=4)
            ...
            button_visit_amzn = Button(self.window, text="visit site", command=self.visit_amzn, bd=4)
            button_visit_amzn.grid(row=4, column=2, sticky=W)

            button_visit_flip = Button(self.window, text="visit site", command=self.visit_flip, bd=4)
            button_visit_flip.grid(row=1, column=2, sticky=W)
        ...
        def price_amzn(self, key):
            ...
            user_input = self.var.get().title()
            self.matches_amzn = get_close_matches(user_input,list(map.keys()), 20, 0.1)
            self.looktable_amzn = {}
        ...
        def search(self):
            ...
            price,self.product_link = self.looktable_amzn[product][0], self.looktable_amzn[product][1]
